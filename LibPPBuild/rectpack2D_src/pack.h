...
3. bool packRecArray(rect_xywhf* const * v, int n, int max_side, std::vector<bin>& bins) - actual packing function
	Arguments:
...
struct rect_wh {
    ...
    int w, h, area(), perimeter(),
            fits(const rect_wh &bigger, bool allowFlipp) const; // 0 - no, 1 - yes, 2 - flipped, 3 - perfectly, 4 perfectly flipped
};
...
bool pack(rect_xywhf *const *v, int n, int max_side, bool allowFlipp, std::vector<bin> &bins);
